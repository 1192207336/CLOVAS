...
class SegPlusCriterion(nn.Module):
    ...
    def __init__(self, 
        ...
        self.loss_map={"masks": self.loss_masks,"binary_anomaly_masks":self.loss_binary_masks,"images":self.loss_images,"llm_const":self.loss_llm_const} # ,"contrastive":self.loss_contrastive
    ...
    def loss_masks(self, outputs, targets, indices, num_masks):
        ...
        # for focal loss
        src_masks = outputs["pred_masks"] # [bs,c_max,h,w]
        ...
        losses = {
            "loss_focal": sigmoid_focal_loss(src_masks, target_masks, num_masks),
            "loss_dice": dice_loss(src_masks_dice, target_masks_dice, num_masks),
        }
   
        return losses
    def loss_binary_masks(self,outputs,targets, indices, num_masks):
        gt_masks = torch.stack([t["binary_mask"] for t in targets], dim=0)
        similarity_map_list = outputs['similarity_map_list']
        focal_loss = 0
        dice_loss = 0
        ...
        return losses
    def loss_images(self,outputs,targets, indices, num_masks):
        text_probs = outputs['text_probs']
        labels = torch.stack([t["is_anomaly_label"] for t in targets], dim=0)
        loss = F.cross_entropy(text_probs, labels.long())
        return {"loss_constractive":loss}
    def loss_llm_const(self,outputs,targets, indices, num_masks):
        llm_text_probs = outputs['llm_text_probs']
        bs = llm_text_probs.shape[0]
        labels = torch.zeros(bs, self.num_classes).to(llm_text_probs.device)
        for i in range(len(targets)):
            labels[i][targets[i]["labels"]] = 1
        labels = labels.unsqueeze(1).unsqueeze(1)
        loss = nn.BCEWithLogitsLoss()(llm_text_probs, labels)
        # loss = F.cross_entropy(llm_text_probs, labels)
        return {"loss_llm_const":loss}
    def _get_target_mask_binary_cross_entropy(self, out_masks, targets):
        B, C = out_masks.size()[:2] # b,c+1
        # print("targets是：",targets)
        # print("targets是：",targets)
        # H, W = targets[0]['masks'].size()
        hw = targets[0]['masks']
        if hw.ndim == 1:
            side = int(hw.shape[0] ** 0.5)
            H, W = side, side
        else:
            H, W = hw.shape
        ...
        #     target_masks_o[i, mask, idx] = 1
        for i, target in enumerate(targets):
            mask = target['masks'].long().reshape(-1)
            # print(f"H*W:{H*W}, out_masks:{out_masks}")
            idx = torch.arange(0, H*W, 1).long().to(out_masks.device) 
            mask_o = mask[mask!=self.ignore_index]
            idx = idx[mask!=self.ignore_index]
...
class SegLossPlus(nn.Module):
    ...
    def __init__(self,
        ...
        weight_dict = {"loss_focal": loss_weight["focal_weight"], 
                       "loss_dice": loss_weight["dice_weight"],
                       "loss_binary_focal":loss_weight["binary_focal_weight"],
                       "loss_binary_dice":loss_weight["binary_dice_weight"],
                       "loss_constractive":loss_weight["image_weight"],
                       "loss_llm_const":loss_weight["llm_const_weight"],} 
        self.losses = losses
        self.weight_dict = weight_dict
        ...
        self.loss_weight = loss_weight["total_loss_weight"]
        self.ignore_index = ignore_index
    ...
    def prepare_targets(self, targets):
        ...
        for targets_per_image,label_per_image in zip(target_masks, target_labels):
            # gt_cls
            binary_gt = targets_per_image.clone()
            binary_gt[binary_gt > 0] = 1
            if self.remove_background:
                targets_per_image[targets_per_image == 0] = 255
                targets_per_image=targets_per_image-1
                targets_per_image[targets_per_image==254] = 255
            ...
            out = {
                ...
                "binary_mask": binary_gt,
                "is_anomaly_label":label_per_image
            }
