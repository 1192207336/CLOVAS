...
class ResidualAttentionBlock(nn.Module):
    ...
    def attention(self, x: torch.Tensor):
        if isinstance(self.attn, Attention_Surgery):
            x = x.transpose(0, 1)
            x, x_ori = self.attn(x)
    ...
    def forward(self, x: torch.Tensor, return_attention: bool=False, ffn = False):
...
class Transformer(nn.Module):
    ...
    def CLIP_Surgery_forward(self, x, out_layers, ffn):
        idx = 0
        out_tokens = []
    ...
    def forward(self, x: torch.Tensor, out_layers = [3, 5, 7, 11], DPAM_layer = None, ffn = False):
        ...
        if DPAM_layer is None:
            ...
            x, out_tokens = self.CLIP_Surgery_forward(x, out_layers, ffn)
            return x, out_tokens
...
class Attention_Surgery(nn.Module):
