...
class CLIPTextEncoder(nn.Module):
    ...
    def forward(self, text):
        ...
        return x
class TextEncoder_simple(nn.Module):
...
class CoOpTextEncoder(nn.Module):
    ...
    def build_attention_mask(self):
        ...
        return mask
    def forward(self, prompts, tokenized_prompts):
        x = prompts + self.positional_embedding.type(prompts.dtype) # prompts:c,77,512  tokenized_prompts:c,77
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        x = self.ln_final(x).type(self.dtype)
        ...
        # x.shape = [batch_size, n_ctx, transformer.width]
        # take features from the eot embedding (eot_token is the highest number in each sequence)
        x = x[torch.arange(x.shape[0]), tokenized_prompts.argmax(dim=-1)] @ self.text_projection # c,512
