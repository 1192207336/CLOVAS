...
def load(name: str,configs:dict, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", design_details = None, jit: bool = False, download_root: str = None):
    """Load a CLIP model
    ...
    if name in _MODELS:
        ...
        model_path = _download(_MODELS[name], download_root or os.path.expanduser(os.path.join(cache_dir,"clip")))
    ...
    if not jit:
        model = build_model(name, state_dict or model.state_dict(),configs = configs)
        model.to(device)
        if str(device) == "cpu":
            model.float()
    ...
    # clip_text_encoder.apply(patch_device)
