...
class AnomalyCLIP_PromptLearner_without_tpt(nn.Module):
    def __init__(self, clip_model, n_ctx = 12,cocoop_mode=False):
        ...
        if ctx_init_pos and ctx_init_neg:
            ...
            if True:
                ...
                ctx_vectors_pos = torch.empty(self.n_cls, n_ctx_pos,ctx_dim, dtype=dtype)
                ctx_vectors_neg = torch.empty(self.n_cls, n_ctx_neg, ctx_dim, dtype=dtype)
            else:
    ...
    def forward_meta_net(self,im_features):
        ...
        prefix_pos = self.token_prefix_pos # 1,n_ctx,dim
